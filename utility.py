...
def get_dataset():
    ...
    print("🌐 MT5 DATA HARVESTER: Inizializzazione Connessione Forex 🌐")

    # 1. Inizializza la connessione a MetaTrader 5
    if not mt5.initialize():
        print("❌ Connessione a MT5 fallita. Assicurati che MT5 sia aperto.")
        mt5.shutdown()
        quit()

    print("✅ Connessione a MT5 stabilita con successo.")

    # NOME DEL SIMBOLO (Cambialo se il tuo broker usa suffissi es. EURUSD.a)
    symbol = "EURUSD"

    # 2. Definisci il range temporale (Ultimi 5 anni)
    timezone = pytz.timezone("Etc/UTC")
    utc_from = datetime(2021, 1, 1, tzinfo=timezone)
    utc_to = datetime.now(timezone)

    print(f"📥 Scaricamento candele M1 per {symbol} dal {utc_from.date()} al {utc_to.date()}...")

    # 3. Estrazione dei dati
    rates = mt5.copy_rates_range(symbol, mt5.TIMEFRAME_M1, utc_from, utc_to)

    if rates is None or len(rates) == 0:
        print(f"❌ Nessun dato trovato per {symbol}. Controlla il nome del simbolo sul tuo broker.")
        mt5.shutdown()
        quit()

    # Chiudiamo la connessione, i dati sono ormai nel nostro Python
    mt5.shutdown()

    # 4. Trasformazione in DataFrame Pandas
    df = pd.DataFrame(rates)
    df['time'] = pd.to_datetime(df['time'], unit='s') # Converte i secondi nel formato Data/Ora
    df.set_index('time', inplace=True)

    # 5. Pulizia e Calcoli Specifici per il Forex
    # In EURUSD, 1 Pip = 0.0001. I punti (tick) di MT5 sono 0.00001 (Pipettes)
    PIP_VALUE = 0.0001

    print("\n" + "="*80)
    print(f"🧬 RADIOGRAFIA STRUTTURALE: {symbol} 🧬")
    print("="*80)

    print(f"📊 Totale Candele a 1 Minuto scaricate: {len(df):,}")
    print(f"🗓️ Primo dato: {df.index.min()}")
    print(f"🗓️ Ultimo dato:  {df.index.max()}")

    # Calcolo dello Spread Medio (se fornito dal broker)
    if 'spread' in df.columns:
        # Lo spread su MT5 è solitamente in punti (pipettes). Dividiamo per 10 per i Pips.
        avg_spread_pips = (df['spread'].mean()) / 10
        print(f"✂️ Spread Medio Storico: {avg_spread_pips:.2f} Pips")

    # 6. Calcolo dell'Average Daily Range (ADR)
    # Raggruppiamo i dati per singola giornata per vedere quanto si muove in media
    daily_data = df.groupby(df.index.date).agg(
        High=('high', 'max'),
        Low=('low', 'min')
    )
    daily_data['Daily_Range_Pips'] = (daily_data['High'] - daily_data['Low']) / PIP_VALUE

    adr_all_time = daily_data['Daily_Range_Pips'].mean()
    adr_last_90 = daily_data['Daily_Range_Pips'].tail(90).mean()

    print("\n📈 ANALISI DELLA VOLATILITÀ (Average Daily Range - ADR)")
    print(f" - ADR Storico (5 anni):    {adr_all_time:.1f} Pips al giorno")
    print(f" - ADR Recente (ultimi 90g): {adr_last_90:.1f} Pips al giorno")
    print("-" * 80)
    print("💡 NOTA: Se l'ADR recente è più basso di quello storico, il mercato è in compressione.")
    print("="*80)

    # Mostriamo le ultime 5 righe per assicurarci che i prezzi abbiano senso
    print("\nAnteprima ultimi dati:")
    print(df[['open', 'high', 'low', 'close', 'tick_volume']].tail())


def backtest_final(df: pd.DataFrame):
